...
const Index = () => {
  ...
  const fetchApiData = async () => {
    const [res, err] = await logService.findAll();
    if (res) {
  ...
  const handleSearch = async (event) => {
    ...
    const [res, err] = await logService.search(name, sort, page);
    if (res) {
      setApiData(res.data.data);
  ...
  const handlePageChange = async (page) => {
    ...
    setCurrentPage(page);
    const [res, err] = await logService.search(null, "Id-DESC", page);
    if (res) {
      setApiData(res.data.data);
      setTotalPages(res.data.totalPages);
    } else {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      const [res, err] = await logService.remove(id);
      if (res) {
