...
const Index = () => {
  ...
  const fetchApiData = async () => {
    const [res, err] = await productAttributeService.findById(id);
    if (res) {
  ...
  const handleAddBtn = () => {
    navigate(`/admin/product/attribute/create/${id}`);
  };

  const handlePageChange = async (page) => {
    if (page < 1 || page > totalPages) return;
    setCurrentPage(page);
    const [res, err] = await productAttributeService.search(
      null,
      "Id-DESC",
      page
    );
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      const [res, err] = await productAttributeService.remove(id);
      if (res) {
            ...
            <div class="iq-card">
              <div class="iq-card-header d-flex justify-content-between">
                <div class="iq-header-title">
                  <h4 class="card-title">List Attribute</h4>
                </div>
                              ...
                              <tr key={item.productId}>
                                ...
                                <td>
                                  <div className="flex align-items-center list-user-action">
                                    <Link
                                      to={`/admin/product/attribute/edit/${item.productId}/${item.productDetailId}`}
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        handleDelete(item.productDetailId)
                                      }
                                      style={{
                                        fontSize: "16px",
                                        width: "25px",
